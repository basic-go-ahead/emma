...
space_expr = re.compile("\s+")


def _preprocess(s: str) -> str:
    s = space_expr.sub(" ", s)
    s = translit(s, "ru", reversed=True)
    r = set(s) - set(string.punctuation) - set(string.ascii_letters) - set(string.digits) - set(string.whitespace)
    for char in r:
        try:
...
class Moderator:
  ...
  def __init__(self, device: Optional[int] = None, model: str = "basic-go/texbert-moderator"):
