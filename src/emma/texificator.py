...
class Texificator:
    ...
    def __init__(self, device: Optional[str] = None, model: Optional[str] = None):
        ...
        self.corrector = pipeline(
            "text2text-generation", model="ai-forever/sage-fredt5-distilled-95m"
        )
    ...
    def _correct(self, text: str):
        if re.finditer("[А-Яа-яЁё]+", text):
            f = [x.group() for x in re.finditer(r"\\\(([^А-Яа-яЁё]+)\\\)", text)] + [
                x.group() for x in re.finditer(r"\\\[(.+)\\\]", text)
            ]
    ...
    def _generate(self, text: str) -> str:
        inputs = {
            k: v.to(self.device)
            for k, v in self._tokenizer(text, return_tensors="pt").items()
        }
    ...
    def doit(
        ...
        texification_type: TexificationType = "full",
        ...
        if texification_type not in ["full", "web"]:
            raise ValueError(
                f"Указан недопустимый тип тексификации `{texification_type}`."
            )

        if isinstance(items, str):
            return self._retrieve(
                self._generate(self._make_prompt(items, texification_type))
            )
        elif isinstance(items, Iterable):
            return self._retrieve(
                (
                    self._make_prompt(self._generate(item), texification_type)
                    for item in items
                )
            )
